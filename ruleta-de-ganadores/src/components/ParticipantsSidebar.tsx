      ...
      <div className="p-4 border-b border-slate-800 bg-slate-950/60 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-[#006837]/30 text-emerald-300 font-mono text-xs px-2 py-0.5 rounded-full border border-[#006837]">
              {participants.length}
            </span>
          </div>
          <div className="flex items-center gap-1">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${sheetConfig.status === 'loading' ? 'animate-spin text-emerald-400' : ''}`}
              />
            </button>
            <button
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <div className="truncate">
              <div className="flex items-center gap-1.5 font-medium text-slate-200 truncate">
                ...
                <span className="truncate font-semibold text-emerald-300">{activeJornadaName}</span>
              </div>
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full bg-slate-800 border border-slate-700 text-xs text-white rounded-lg px-2.5 py-1.5 appearance-none pr-8 focus:outline-none focus:border-emerald-400 cursor-pointer font-medium"
            >
              {JORNADAS.map((j) => (
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-slate-800 border border-slate-700 text-xs text-white rounded-lg px-2.5 py-1.5 appearance-none pr-8 focus:outline-none focus:border-emerald-400 cursor-pointer"
              >
      ...
      <div className="p-3 border-b border-slate-800 bg-slate-900/50 space-y-2">
        ...
        <div className="flex items-center justify-between text-xs pt-1">
          <button
            ...
            className="flex items-center gap-1 text-slate-300 hover:text-emerald-400 disabled:opacity-50 transition-colors cursor-pointer"
          >
      ...
      <div className="p-3 border-t border-slate-800 bg-slate-950/80">
        <form onSubmit={handleAddSubmit} className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-400"
          />
          <button
            type="submit"
            disabled={!newName.trim()}
            className="bg-[#006837] hover:bg-[#005028] disabled:opacity-50 text-white p-2 rounded-lg transition-colors cursor-pointer"
          >
